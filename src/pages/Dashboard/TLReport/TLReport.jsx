...
export default function TLReport({ data, loading }) {
  ...
  const getSections = () => {
    if (activeTab === "bkt") {
      return [
        ...
        { label: "EMP Name",       cols: BKT_COLS, rows: data?.bkt?.emp ?? [] },
      ];
    } else {
      return [
        { label: "Portfolio Name", cols: REC_PORTFOLIO_COLS, rows: data?.rec?.portfolio ?? [] },
        { label: "EMP Name",       cols: REC_COMMON_COLS,    rows: data?.rec?.emp ?? [] },
      ];
    }
  };
  ...
  return (
    <div className="tl-report-card">
      <div className="tl-report-header">
        <h2 className="tl-report-title">TL Report</h2>
      </div>

      <RoleTabs tabs={TABS} activeTab={activeTab} onChange={setActiveTab} />

      <div className="tl-report-body" style={{ minHeight: 250, position: "relative" }}>
        {loading ? (
